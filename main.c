...
unsigned int i=0,j=0,k=0, crd=0, curs=0, kee=0;
unsigned int inibet=0;

unsigned int rd1=0, rd2=0, nb1=0, nb2=0, crd1=0, crd2=0;
	...
	while(cont)
	{
		...
		for(i=0;i<nbPlayers;i++){
			...
			if(p[i].play) {
				...
				//sprintf(message,"cartes du joueur:%d %d",p[i].hand1, p[i].hand2);
				...
				while(selec)
				{
					...
					AdvancedPrint(10,25,"VALIDER"); APlot(30,24,' ',6); AdvancedPrint(30,24,itoa(p[i].bet));
					AdvancedPrint(9,27,"  [FLECHES]  [ENTER]  ");
